...
dataset = pd.read_csv(proj_dir / 'data' / 'kor_pair_train.csv').filter(items=['question1', 'question2',
                                                                              'is_duplicate'])
dataset['is_duplicate'] = dataset['is_duplicate'].apply(lambda elm: 1 - elm)
tst = pd.read_csv(proj_dir / 'data' / 'kor_pair_test.csv').filter(items=['question1', 'question2',
                                                                         'is_duplicate'])
tst['is_duplicate'] = tst['is_duplicate'].apply(lambda elm: 1 - elm)
...
vocab = nlp.Vocab(counter=counter, min_freq=1, bos_token=None, eos_token=None)
...
tr, val = train_test_split(dataset, test_size=.2, random_state=777)
