...
def get_data_loaders(dataset_config, model_config, tokenizer, batch_size, collate_fn=batchify):
    tr_ds = Corpus(dataset_config.train, tokenizer.split_and_transform, min_length=model_config.min_length,
                   pad_val=tokenizer.vocab.to_indices(' '))
    tr_dl = DataLoader(tr_ds, batch_size=batch_size, shuffle=True, num_workers=4,
                       collate_fn=collate_fn, drop_last=True)
    val_ds = Corpus(dataset_config.validation, tokenizer.split_and_transform, min_length=model_config.min_length,
                   pad_val=tokenizer.vocab.to_indices(' '))
...
def main(args):
    ...
    tr_dl, val_dl = get_data_loaders(dataset_config, model_config, tokenizer, args.batch_size)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help="directory containing convrec.json",
    )
