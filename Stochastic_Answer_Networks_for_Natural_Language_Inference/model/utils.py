...
class PreProcessor:
    def __init__(self, coarse_vocab, fine_vocab, coarse_split_fn, fine_split_fn, fine_max_len=12):
        ...
        self._fine_split_fn = fine_split_fn
        self._fine_pad_fn = PadSequence(length=fine_max_len, pad_val=self._fine_vocab.padding_token, clip=True)

    def preprocess(self, string):
        list_of_coarse = self._coarse_split_fn(string)
        list_of_fines_indices = [self._transform_fine(coarse) for coarse in list_of_coarse]
    ...
    def _transform_fine(self, token):
        if token == self._coarse_vocab.padding_token:
            ...
            list_of_fine_indices = self._fine_vocab.to_indices(list_of_fine)
            return list_of_fine_indices
        elif token == self._coarse_vocab.unknown_token:
            list_of_fine = [self._fine_vocab.unknown_token] * self._fine_pad_fn._length
            list_of_fine_indices = self._fine_vocab.to_indices(list_of_fine)
            return list_of_fine_indices
        else:
            list_of_fine = self._fine_pad_fn(self._fine_split_fn(token))
            list_of_fine_indices = self._fine_vocab.to_indices(list_of_fine)
